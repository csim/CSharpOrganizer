  ...
  return new Promise((resolve, reject) => {
    ...
    client.on("error", async (error) => {
      ...
      if (
        ...
        try {
          console.log("Daemon connection failed, attempting to restart...");
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      daemonProcess.on("spawn", async () => {
        console.log("CSharply daemon started");
        ...
        if (waited >= maxWait) {
          console.error("Daemon started but pipe is not available");
          reject(new Error("Daemon started but pipe is not available"));
        } else {
          console.log("Daemon pipe is ready");
          resolve();
        }
      });
...
export function stop(): void {
  if (daemonProcess) {
    try {
      ...
      // If process doesn't exit gracefully in 5 seconds, force kill
      setTimeout(() => {
        if (daemonProcess && !daemonProcess.killed) {
          console.log("Daemon didn't exit gracefully, force killing...");
